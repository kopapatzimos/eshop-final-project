...
CATEGORY_CHOICES = (("G", "Guitars"), ("B", "Basses"))
...
class Item(models.Model):
    ...
    def average_rating(self):
        rt = int(self.review_set.count())

        # if rt not empty(if there are review for this item)
        if rt != int(0):
            # print("not empty")
            return int(self.review_set.aggregate(Avg("rating"))["rating__avg"])
        # otherwise return 0
        return int(0)
...
class Wishlist(models.Model):
    ...
    def get_total(self):
        total = 0
        for wish_item in self.items.all():
            total += wish_item.get_final_price()
        if self.coupon:
